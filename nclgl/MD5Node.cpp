...
void	MD5Node::Update(float msec) {
	...
	if (isBakeIntoPose)
		worldTransform = worldTransform * currentSkeleton.joints->transform.GetInversedMatrix();
}
...
void	MD5Node::Draw() {
	...
	*/
	...
	m->SetMatrial(material);

	sourceData.BindTextureBuffers();
	sourceData.UpdateTransformTBO(currentSkeleton);

	glUseProgram(m->GetMatrial()->GetShader()->GetProgram());
	glUniform1i(glGetUniformLocation(m->GetMatrial()->GetShader()->GetProgram(), "weightTex"), MD5_WEIGHT_TEXNUM);
	glUniform1i(glGetUniformLocation(m->GetMatrial()->GetShader()->GetProgram(), "transformTex"), MD5_TRANSFORM_TEXNUM);
