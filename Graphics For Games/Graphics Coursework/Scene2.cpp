...
void Scene2::InitShaderMaterial()
{
...
void Scene2::RenderScene(GLuint _renderTarget , GLuint _depthMap , GLuint _FBO)
{
...
void Scene2::UpdateScene(float msec)
{
	//camera comtrol
	if (isAutoRotate)
	{
...
void Scene2::DrawNode(SceneNode *n)
{
	if (n->GetMesh())
	{
...
void Scene2::DrawUI()
{
	DrawText(FPS, Vector3(0, 0, 0), 20.0f,false);
}

void Scene2::SetRenderOrder(SceneNode * from)
{
...
void Scene2::ClearNodeLists()
{
	transparentNodeList.clear();
	nodeList.clear();
}

void Scene2::DrawNodes()
{
...
void Scene2::InitMonster()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		if (i == 2)
			skeletonNode->SetTransform(Matrix4::Translation(Vector3(-300.0f + i * 200.0f, 0.0f, 200.0f + i * -200.0f))*Matrix4::Rotation(-90.0f,Vector3(0.0f,1.0f,0.0f)));
		else
...
*/
void Scene2::DrawText(const std::string &text, const Vector3 &position, const float size, const bool perspective) {
